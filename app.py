...
# ARIMA model training with fixed (p, d, q) order of (2, 1, 2)
st.subheader("Training the ARIMA Model")
arima_order = (2, 1, 2)  # Fixed order
...
if future_steps > 0:
    ...
    ax3.plot(stock_prices.index, stock_prices, label='Historical Prices', color='purple')
    ax3.plot(future_dates, future_predictions, label='Future Predicted Prices', linestyle='--', color='red')
    ax3.set_xlabel('Date')
    ...
    ax5.plot(stock_prices.index, stock_prices, label='Historical Prices', color='green')
    ax5.plot(next_30_dates, next_30_days, label='Next 30 Days Predictions', linestyle='--', color='lightblue')
    ax5.set_xlabel('Date')
